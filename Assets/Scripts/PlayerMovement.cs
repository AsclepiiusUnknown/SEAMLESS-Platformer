...
public class PlayerMovement : PlayerController
{
    ...
    private void Update()
    {
        ...
        if (!isTouchingLedge && isTouchingWall && Input.GetKey(KeyCode.Y) && !isDashing) //If we are touching the wall but not the ledge (aka at the top of the wall/ledge)
        {
        ...
        {
            playerMovement.rb.velocity = Vector2.zero; //stop moving
        }

        if (playerCollision.isLedgeGrabbing && Input.GetKeyDown(KeyCode.U))
        {
            if (playerMovement.xRawInput != 0 || playerMovement.yRawInput != 0)
            {
                playerCollision.isLedgeGrabbing = false;
                Dash(playerMovement.xRawInput, playerMovement.yRawInput);
            }
        }
        ...
        #region Wall Sliding
        if (isTouchingWall && !playerCollision.isGrounded && playerMovement.xRawInput != 0 && !playerCollision.isLedgeGrabbing) //if we are touching the wall, arent grounded, moving (towards the wall), and not grabbing the ledge
        {
        ...
        {
            playerMovement.rb.velocity = new Vector2(playerMovement.rb.velocity.x, Mathf.Clamp(playerMovement.rb.velocity.y, -wallSlidingSpeed, float.MaxValue)); //Move down the wall at a speed clamped between sliding speed and the max possble float
        }
        #endregion
        ...
        if (isWallJumping)
        {
            playerMovement.rb.velocity = new Vector2(xWJForce * -playerMovement.xRawInput, yWJForce);
        }
        #endregion
    }
